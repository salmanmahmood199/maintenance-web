  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      if (userData.role === 'subadmin') {
        // Get subAdmin data
        const subAdminResponse = await fetch(`/subAdmins/email/${encodeURIComponent(email)}`);
        ...
        // Get vendor data
        const vendorResponse = await fetch(`/vendors/email/${encodeURIComponent(email)}`);
        ...
        // Get technician data
        const techResponse = await fetch(`/technicians?email=${encodeURIComponent(email)}`);
        if (techResponse.ok) {
